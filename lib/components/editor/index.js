  	...
  	this.setState({
		...
		value: function _insertAsyncBlockComponent(type, file, props) {
			...
			(0, _fileUpload.getSignature)(file).done(function (jsonDataForUpload) {
				...
				(0, _fileUpload.uploadToS3)(jsonDataForUpload, file).done(function () {
					(0, _fileUpload.getFileUrl)(jsonDataForUpload).done(function (res) {
		...
		value: function _handleFileInput(e) {
			...
			files.forEach(function (f) {
				...
				console.log(f);
				if (f.type.indexOf('image') > -1) _this2._insertAsyncBlockComponent("IMAGE", f, props);else if (f.type.indexOf('video') > -1) _this2._insertAsyncBlockComponent("VIDEO", f, props);else if (f.type.indexOf('document') > -1) _this2._insertAsyncBlockComponent("DOCUMENT", f, props);else if (f.type.indexOf('audio') > -1) _this2._insertAsyncBlockComponent("AUDIO", f, props);
			});
		}
	}, {
