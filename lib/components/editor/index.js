...
var Link = function Link(props) {
	...
	return _react2.default.createElement(
		'a',
		{ href: url, style: styleLink, target: '_blank' },
		props.children
	);
};
...
var RichEditor = function (_Component) {
	...
	function RichEditor(props) {
		...
		_this.setLoadingState = function (counter) {
			_this.uploading = counter;
			if (_this.props.onUploadStatusChange) _this.props.onUploadStatusChange(_this.uploading);
		};
		return _this;
	}
	...
	_createClass(RichEditor, [{
		...
		value: function _insertAsyncBlockComponent(type, file, props) {
			...
			var entityKey = this._insertBlockComponent(null, type, props);
		...
		value: function _handleHyperLink(url) {
			...
			(0, _fileUpload.getURLData)(props.url).done(function (res) {
				//console.log(res);
		...
		key: '_linkFail',
		value: function _linkFail(props) {
			var that = this;
		...
		value: function _handleFileInput(e) {
			...
			var docReg = /(doc|docx|pdf|wps|xls|xlsx)/i;
		...
		value: function _handlePaste(text) {
			...
			if (youtubeTest) {
				...
				setTimeout(function () {
					_this3._insertBlockComponent(null, "YOUTUBE", { src: youtubeTest[0], file: youtubeTest[1], url: text });
				}, 500);
